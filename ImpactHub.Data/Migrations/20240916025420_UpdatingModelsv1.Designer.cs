// <auto-generated />
using System;
using ImpactHub.Data.Contexts;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Oracle.EntityFrameworkCore.Metadata;

#nullable disable

namespace ImpactHub.Data.Migrations
{
    [DbContext(typeof(ImpactHubDbContext))]
    [Migration("20240916025420_UpdatingModelsv1")]
    partial class UpdatingModelsv1
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            OracleModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("ImpactHub.Business.Models.CadastroModel", b =>
                {
                    b.Property<int>("IdCadastro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdCadastro"));

                    b.Property<string>("Cnpj")
                        .IsRequired()
                        .HasColumnType("varchar(14)");

                    b.Property<DateTime>("DataAbertura")
                        .HasColumnType("TIMESTAMP(7)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("varchar(30)");

                    b.Property<string>("InscricaoEstadual")
                        .IsRequired()
                        .HasColumnType("varchar(8)");

                    b.Property<string>("NomeEmpresa")
                        .IsRequired()
                        .HasColumnType("varchar(200)");

                    b.Property<string>("NomeUsuario")
                        .IsRequired()
                        .HasColumnType("varchar(100)");

                    b.Property<string>("Porte")
                        .IsRequired()
                        .HasColumnType("varchar(10)");

                    b.Property<string>("RazaoSocial")
                        .IsRequired()
                        .HasColumnType("varchar(180)");

                    b.Property<string>("Senha")
                        .IsRequired()
                        .HasColumnType("varchar(100)");

                    b.HasKey("IdCadastro");

                    b.ToTable("TB_IMPACTHUB_CADASTRO", (string)null);
                });

            modelBuilder.Entity("ImpactHub.Business.Models.ContatoModel", b =>
                {
                    b.Property<int>("IdContato")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdContato"));

                    b.Property<int>("Ddd")
                        .HasMaxLength(3)
                        .HasColumnType("NUMBER(10)");

                    b.Property<int>("Ddi")
                        .HasMaxLength(3)
                        .HasColumnType("NUMBER(10)");

                    b.Property<string>("StatusContato")
                        .IsRequired()
                        .HasColumnType("varchar(25)");

                    b.Property<string>("Telefone")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("TipoContato")
                        .IsRequired()
                        .HasColumnType("varchar(25)");

                    b.HasKey("IdContato");

                    b.ToTable("TB_IMPACTHUB_CONTATO", (string)null);
                });

            modelBuilder.Entity("ImpactHub.Business.Models.EnderecoModel", b =>
                {
                    b.Property<int>("IdEndereco")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdEndereco"));

                    b.Property<string>("Bairro")
                        .IsRequired()
                        .HasColumnType("varchar(80)");

                    b.Property<string>("Cep")
                        .IsRequired()
                        .HasColumnType("varchar(10)");

                    b.Property<string>("Cidade")
                        .IsRequired()
                        .HasColumnType("varchar(80)");

                    b.Property<string>("Complemento")
                        .HasColumnType("varchar(125)");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasColumnType("varchar(2)");

                    b.Property<string>("Logradouro")
                        .IsRequired()
                        .HasColumnType("varchar(125)");

                    b.Property<int>("Numero")
                        .HasMaxLength(5)
                        .HasColumnType("NUMBER(10)");

                    b.Property<string>("PontoReferencia")
                        .HasColumnType("varchar(150)");

                    b.HasKey("IdEndereco");

                    b.ToTable("TB_IMPACTHUB_ENDERECO", (string)null);
                });

            modelBuilder.Entity("ImpactHub.Business.Models.LoginModel", b =>
                {
                    b.Property<int>("IdLogin")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdLogin"));

                    b.Property<string>("NomeUsuario")
                        .IsRequired()
                        .HasColumnType("varchar(80)");

                    b.Property<string>("Senha")
                        .IsRequired()
                        .HasColumnType("varchar(16)");

                    b.Property<string>("StatusLogin")
                        .IsRequired()
                        .HasColumnType("varchar(20)");

                    b.HasKey("IdLogin");

                    b.ToTable("TB_IMPACTHUB_LOGIN", (string)null);
                });

            modelBuilder.Entity("ImpactHub.Business.Models.MonitoramentoModel", b =>
                {
                    b.Property<int>("IdMonitoramento")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdMonitoramento"));

                    b.Property<DateTime>("DataValidade")
                        .HasColumnType("TIMESTAMP(7)");

                    b.Property<string>("DescricaoMonitoramento")
                        .IsRequired()
                        .HasColumnType("varchar(20)");

                    b.Property<int>("IdLogin")
                        .HasColumnType("NUMBER(10)");

                    b.Property<int>("IdResultado")
                        .HasColumnType("NUMBER(10)");

                    b.Property<int>("LoginIdLogin")
                        .HasColumnType("NUMBER(10)");

                    b.Property<string>("StatusMonitoramento")
                        .IsRequired()
                        .HasColumnType("varchar(15)");

                    b.HasKey("IdMonitoramento");

                    b.HasIndex("IdResultado");

                    b.HasIndex("LoginIdLogin");

                    b.ToTable("TB_IMPACTHUB_MONITORAMENTO", (string)null);
                });

            modelBuilder.Entity("ImpactHub.Business.Models.QuestionarioModel", b =>
                {
                    b.Property<int>("IdQuestionario")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdQuestionario"));

                    b.Property<string>("DescricaoPergunta")
                        .IsRequired()
                        .HasColumnType("varchar(225)");

                    b.Property<string>("DescricaoResposta")
                        .IsRequired()
                        .HasColumnType("varchar(225)");

                    b.Property<int>("IdTipoQuestionario")
                        .HasColumnType("NUMBER(10)");

                    b.HasKey("IdQuestionario");

                    b.HasIndex("IdTipoQuestionario");

                    b.ToTable("TB_IMPACTHUB_QUESTIONARIO", (string)null);
                });

            modelBuilder.Entity("ImpactHub.Business.Models.RankingModel", b =>
                {
                    b.Property<int>("IdResultado")
                        .HasColumnType("NUMBER(10)");

                    b.Property<int>("IdRanking")
                        .HasColumnType("NUMBER(10)");

                    b.Property<int>("Posicao")
                        .HasColumnType("NUMBER(10)");

                    b.HasKey("IdResultado");

                    b.ToTable("TB_IMPACTHUB_RANKINGS", (string)null);
                });

            modelBuilder.Entity("ImpactHub.Business.Models.RelatorioModel", b =>
                {
                    b.Property<int>("IdRelatorio")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdRelatorio"));

                    b.Property<int>("IdLogin")
                        .HasColumnType("NUMBER(10)");

                    b.Property<int>("IdResultado")
                        .HasColumnType("NUMBER(10)");

                    b.Property<int>("LoginIdLogin")
                        .HasColumnType("NUMBER(10)");

                    b.Property<string>("Melhorias")
                        .IsRequired()
                        .HasColumnType("varchar(225)");

                    b.Property<string>("PontosFaltantesMelhorias")
                        .IsRequired()
                        .HasColumnType("varchar(225)");

                    b.Property<string>("StatusRelatorio")
                        .IsRequired()
                        .HasColumnType("varchar(20)");

                    b.HasKey("IdRelatorio");

                    b.HasIndex("IdResultado");

                    b.HasIndex("LoginIdLogin");

                    b.ToTable("TB_IMPACTHUB_RELATORIO", (string)null);
                });

            modelBuilder.Entity("ImpactHub.Business.Models.ResultadoModel", b =>
                {
                    b.Property<int>("IdResultado")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdResultado"));

                    b.Property<DateTime>("DataValidade")
                        .HasColumnType("TIMESTAMP(7)");

                    b.Property<int>("IdQuestionario")
                        .HasColumnType("NUMBER(10)");

                    b.Property<string>("PorcentagemCertificado")
                        .IsRequired()
                        .HasColumnType("varchar(5)");

                    b.Property<string>("StatusResultado")
                        .IsRequired()
                        .HasColumnType("varchar(15)");

                    b.HasKey("IdResultado");

                    b.HasIndex("IdQuestionario");

                    b.ToTable("TB_IMPACTHUB_RESULTADOS", (string)null);
                });

            modelBuilder.Entity("ImpactHub.Business.Models.TipoQuestionarioESGModel", b =>
                {
                    b.Property<int>("IdTipoQuestionario")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("NUMBER(10)");

                    OraclePropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdTipoQuestionario"));

                    b.Property<string>("Descricao")
                        .IsRequired()
                        .HasColumnType("varchar(80)");

                    b.Property<string>("TipoQuestionario")
                        .IsRequired()
                        .HasColumnType("varchar(15)");

                    b.HasKey("IdTipoQuestionario");

                    b.ToTable("TB_IMPACTHUB_TIPO_QUESTIONARIO)", (string)null);
                });

            modelBuilder.Entity("ImpactHub.Business.Models.MonitoramentoModel", b =>
                {
                    b.HasOne("ImpactHub.Business.Models.ResultadoModel", "Resultado")
                        .WithMany("Monitoramentos")
                        .HasForeignKey("IdResultado")
                        .IsRequired();

                    b.HasOne("ImpactHub.Business.Models.LoginModel", "Login")
                        .WithMany()
                        .HasForeignKey("LoginIdLogin")
                        .IsRequired();

                    b.Navigation("Login");

                    b.Navigation("Resultado");
                });

            modelBuilder.Entity("ImpactHub.Business.Models.QuestionarioModel", b =>
                {
                    b.HasOne("ImpactHub.Business.Models.TipoQuestionarioESGModel", "TipoQuestionario")
                        .WithMany("Questionarios")
                        .HasForeignKey("IdTipoQuestionario")
                        .IsRequired();

                    b.Navigation("TipoQuestionario");
                });

            modelBuilder.Entity("ImpactHub.Business.Models.RankingModel", b =>
                {
                    b.HasOne("ImpactHub.Business.Models.ResultadoModel", "Resultado")
                        .WithMany("Rankings")
                        .HasForeignKey("IdResultado")
                        .IsRequired();

                    b.Navigation("Resultado");
                });

            modelBuilder.Entity("ImpactHub.Business.Models.RelatorioModel", b =>
                {
                    b.HasOne("ImpactHub.Business.Models.ResultadoModel", "Resultado")
                        .WithMany("Relatorios")
                        .HasForeignKey("IdResultado")
                        .IsRequired();

                    b.HasOne("ImpactHub.Business.Models.LoginModel", "Login")
                        .WithMany()
                        .HasForeignKey("LoginIdLogin")
                        .IsRequired();

                    b.Navigation("Login");

                    b.Navigation("Resultado");
                });

            modelBuilder.Entity("ImpactHub.Business.Models.ResultadoModel", b =>
                {
                    b.HasOne("ImpactHub.Business.Models.QuestionarioModel", "Questionario")
                        .WithMany("Resultados")
                        .HasForeignKey("IdQuestionario")
                        .IsRequired();

                    b.Navigation("Questionario");
                });

            modelBuilder.Entity("ImpactHub.Business.Models.QuestionarioModel", b =>
                {
                    b.Navigation("Resultados");
                });

            modelBuilder.Entity("ImpactHub.Business.Models.ResultadoModel", b =>
                {
                    b.Navigation("Monitoramentos");

                    b.Navigation("Rankings");

                    b.Navigation("Relatorios");
                });

            modelBuilder.Entity("ImpactHub.Business.Models.TipoQuestionarioESGModel", b =>
                {
                    b.Navigation("Questionarios");
                });
#pragma warning restore 612, 618
        }
    }
}
